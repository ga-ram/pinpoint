...
public abstract class PinotAlarmChecker<T> {
    ...
    public boolean[] check(long timeSlotEndTime) {
        ...
        T value = dataCollector.collect(serviceName, applicationName, target, range);

        for (int i = 0; i < rules.size(); i++) {
            PinotAlarmRule rule = rules.get(i);
            BigDecimal threshold = rule.getThreshold();

            boolean result = decideResult(threshold, value);
            ...
            logger.info("{} result is {} for application ({}). (threshold : {}, condition : {}, value: {}).",
                    ...
                    threshold, rule.getCondition(), value);
        }
    ...
    public List<String> getSmsMessage(int index) {
        ...
        messages.add(String.format("[PINPOINT Alarm - %s] %s is %s%s (Condition: %s, Threshold : %s%s)",
                applicationName, rules.get(index).getCheckerName(), collectedValue, unit,
                alarmCondition, rules.get(index).getThreshold(), unit));
        return messages;
    }
    
    public String getEmailMessage(int index) {
        return String.format("%s %s value is %s%s during the past 5 mins.(Condition: %s, Threshold : %s%s)<br>",
                applicationName, rules.get(index).getCheckerName(), collectedValue, unit,
                rules.get(index).getCondition(), rules.get(index).getThreshold(), unit);
    }
